...
## CRAN (or mirror) package page
  ...
  stub_request("get", uri_regex = ".+/web/packages/[^/]*") %>%
    to_return(
      body = file(file.path(test_path(), "test_webmocks", "data", "cran_package.html")),
      headers = list(
...
## CRAN (or mirror) NEWS page
  ...
  stub_request("get", uri_regex = ".+/web/packages/[^/]*/news/news.html") %>%
    to_return(
      body = file(file.path(test_path(), "test_webmocks", "data", "cran_news.html")),
      headers = list(
        "Content-Type" = "text/html; charset=utf-8",
        "Content-Encoding" = "UTF-8"))
...
## CRAN (or mirror) package archive listing page
  ...
  stub_request("get", uri_regex = ".+/src/contrib/Archive/[^/]*") %>%
    to_return(
      body = file(file.path(test_path(), "test_webmocks", "data", "ran_package_archive.html")),
      headers = list(
        "Content-Type" = "text/html; charset=utf-8",
        "Content-Encoding" = "UTF-8"))
...
stub_request("get", uri_regex = "api\\.github\\.com/repos/[^/]+/[^/]+/issues") %>%
  ...
  to_return(
    body = file(file.path(test_path(), "test_webmocks", "data", "github_repo_issues_api_response.json")),
