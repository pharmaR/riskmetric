...
test_that("assess_export_help returns expected result for source packages", {
  ...
  expect_equal(unclass(unname(assess_source_good$export_help)), c(TRUE))
  expect_equal(unclass(unname(assess_install_good$export_help)), c(TRUE))

  expect_equal(unclass(unname(assess_source_bad$export_help)), c(FALSE, TRUE))
